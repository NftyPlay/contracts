...
contract LinearVesting is ReentrancyGuard {
    ...
    function _createVestingSchedule(address _beneficiary, uint256 _amount) internal returns (bool) {
        ...
        // transfer 10% to the beneficiary instantly
        require(
            token.transferFrom(msg.sender, _beneficiary, _amount.div(10)),
            "VestingContract::createVestingSchedule: Unable to transfer tokens"
        );

        vestedAmount[_beneficiary] = (_amount.mul(9)).div(10) ;

        // Vest the tokens into the deposit account and delegate to the beneficiary
        require(
            token.transferFrom(msg.sender, address(this), (_amount.mul(9)).div(10)),
    ...
    function _availableDrawDownAmount(address _beneficiary) internal view returns (uint256 _amount) {
        ...
        uint256 drawDownRate = vestedAmount[_beneficiary].div(365*24*60*60);
